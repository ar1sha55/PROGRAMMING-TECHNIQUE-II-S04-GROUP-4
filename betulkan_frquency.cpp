...
class Frequency
{
    ...
    public:
        ...
        // MUTATOR 
        void setFrequency(int f){ freqVal = f; }

        //POLYMORPHISM
        // default print from parent class
        virtual void printFreq(int j)
        {
            cout << "Frequency : " << j << " each time\n";
        }
...
class dailyFreq : public Frequency
{
    ...
    public:
        ...
        // MUTATORS
        void settdailyIntake(int d){ dailyIntake = d; }

        //PRINT DAILY FREQUENCY (POLYMORPHISM)
        void printFreq(int j) override
        {
...
class weeklyFreq : public Frequency  //inheritance
{
    ...
    public:
        ...
        //MUTATORS
        void setdayPerWeek(int i) { dayPerWeek = i; }

        //PRINT WEEKLY FREQUENCY (POLYMORPHISM)
        void printFreq(int k) override
        {
            cout << "\nThis medicine needs to be taken " << k << " day(s) per week, and\n";
        }
};
...
int main() {
    ...
    dailyFreq *dailyf = new dailyFreq[50];
    weeklyFreq weekf;
    ...
    switch(userOption()) 
    {
         case 1: 
        {
            ...
            for(int i = 0; i < numMed; i++) 
            {
                ...
                freq[i] = freq[i].getFreq();

                weekly[i] = weekf.getdayPerWeek();

                daily[i] = dailyf[i].getdailyIntake();
                for(int j = 0; j < daily[i]; j++)
                {
                    times[j] = dailyf[i].getTime(j);
                }
